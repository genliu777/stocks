...
def print_json_to_txt(name, json_data):
	f = open(name, "w")
	f.write("date open close price_delta ratio min, max volumn account raiot2\n")
#	print "date, open, close, price_delta, ratio, min, max, volumn, account, raiot2"
	hq = (json_data[0])[u'hq']
	for day_hq in hq:
		for item in day_hq:
			f.write(item+" ")
...
def get_market_history_json(dir, url):
	...
	if json_content:
		...
		print_json_to_txt("history" ,json_data)
	os.chdir("..")
...
#	print socks
	for k,v in socks.items():
		print v[0], v[1]
		url = "http://q.stock.sohu.com/hisHq?code=cn_{0}&stat=1&order=D&period=d&callback=historySearchHandler&rt=jsonp&0.5429994754601287".format(v[0])
		get_market_history_json(v[0], url)
